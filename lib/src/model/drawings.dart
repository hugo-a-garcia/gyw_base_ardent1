...
import 'drawing_parts/commands.dart';
import 'drawing_parts/fonts.dart';
import 'drawing_parts/icons.dart';
import 'drawing_parts/screen.dart';
...
abstract class GYWDrawing {
  ...
  const GYWDrawing({this.top = 0, this.left = 0});
...
class TextDrawing extends GYWDrawing {
  ...
  List<GYWBtCommand> _lineToCommands(String line, int top) {
    ...
    final shortColor =
        [
          ...
          color.green,
          color.blue,
        ].map((channel) => (channel ~/ 16).toRadixString(16)).join();
    ...
    return [
      ...
      GYWBtCommand(GYWCharacteristic.ctrlDisplay, controlBytes.toBytes()),
    ];
  }
  ...
  @override
  int get hashCode =>
      Object.hash(text, font, left, top, size, color, maxWidth, maxLines);
...
class BlankScreen extends GYWDrawing {
  ...
  factory BlankScreen.fromJson(Map<String, dynamic> data) {
    return BlankScreen(color: colorFromHex(data["color"] as String?));
  }

  @override
  Map<String, dynamic> toJson() {
    return {"type": type, "color": color != null ? hexFromColor(color!) : null};
  }
...
class IconDrawing extends GYWDrawing {
  ...
  @override
  int get hashCode => Object.hash(iconFilename, left, top, color, scale);
...
class RectangleDrawing extends GYWDrawing {
  ...
  @override
  int get hashCode => Object.hash(left, top, width, height, color);
